...
from PySide6.QtWidgets import (QMainWindow, QVBoxLayout, QHBoxLayout, QWidget,
                               ...
                               QHeaderView, QTableWidgetItem, QDialog, QDialogButtonBox)
...
class ChallengeWindow(QMainWindow):
    ...
    def schedule_vote_at_time(self, challenge, vote_count, timing_spec, task_description=None, *args):
        ...
        try:
            ...
            task_id = f"vote_{challenge.id}_{timing_spec}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def on_job_finished(self, event):
        ...
        try:
            ...
            # Extraire l'ID du challenge depuis le job_id (format: vote_CHALLENGE_ID_timestamp)
            ...
            # Vérifier s'il reste d'autres jobs pour ce challenge
            remaining_jobs = self.count_scheduled_votes_for_challenge(challenge_id)
            
            self.log(f"🔍 Job terminé: {job_id} - Jobs restants pour challenge {challenge_id}: {remaining_jobs}")
            
            # S'il n'y a plus de jobs pour ce challenge, nettoyer
            if remaining_jobs == 0:
                self.log(f"🧹 Nettoyage automatique: stratégie terminée pour challenge {challenge_id}")
    ...
    def count_scheduled_votes_for_challenge(self, challenge_id):
        """Compte le nombre de votes programmés pour un challenge"""
        if not hasattr(self, 'scheduler') or not self.scheduler:
            return 0
        ...
        try:
            ...
            count = 0
            for job in jobs:
                if job.id.startswith(f'vote_{challenge_id}_'):
                    count += 1
            return count
        except:
            return 0
    ...
    def remove_scheduler_jobs_for_challenge(self, challenge_id):
        ...
        try:
            ...
            for job in jobs:
                # Vérifier si le job appartient à ce challenge
                if job.id.startswith(f'vote_{challenge_id}_'):
