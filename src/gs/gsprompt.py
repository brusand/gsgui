...
#host = 'localhost'
host = 'influxdb18-liveobjects.kermit-noprod-b.itn.intraorange'
port = 80
user = 'gs-admin'
...
class GuruBatch():
    def __init__(self, player=None):
        ...
        self.parser_challenge.add_argument('--end', action="store_true", default=False)
        ...
        self.parser_fill.add_argument('--below', nargs='?', type=int, action="store", default=100)
    ...
    def init(self, args):
        ...
        #self.connect()
        #self.challenges = ConfigObj('challenges-'+ args.player + '.ini')
        self.threads={}
        self.bye = False
    ...
    def init_process(self, args):
        try:
            ...
            for challenge in challenges["challenges"]:
                    ...
                    if self.player == 'ranking' and  self.challenges[challenge['url']].as_bool('ranking') and datetime.now() <= datetime.strptime(self.challenges[challenge['url']]['end'], "%d/%m/%Y, %H:%M"):
    ...
    def get_votes_panel(self,  url):
        return  self.aio_post('https://api.gurushots.com/rest/get_vote_data', data={
            'limit': 100,
            'url': url
        })
    ...
    async def aio_get_joined_challenges(self):
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False), headers=self.aio_connect_session(), loop=loop) as session:
            response = loop.run_until_complete(
                #self.fetch_page(session, 'https://api.gurushots.com/rest/get_member_joined_active_challenges'))
                self.fetch_page(session, 'https://api.gurushots.com/rest/get_my_active_challenges'))
            print(response)
            return json.loads(response)

    def get_joined_challenges(self):
            return self.session.post('https://api.gurushots.com/rest/get_my_active_challenges',{})
            #return  self.aio_post('https://api.gurushots.com/rest/get_my_active_challenges',{})
            #return  self.aio_post('https://api.gurushots.com/rest/get_member_joined_active_challenges',{})
    ...
    async def aio_get_open_challenges(self):
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False), headers=self.aio_connect_session(), loop=loop) as session:
            response = loop.run_until_complete(
                self.fetch_page_data(session, 'https://api.gurushots.com/rest/get_member_challenges', data={'filter': 'open'}))
            return json.loads(response)


    def get_open_challenges(self):
        return self.aio_post('https://api.gurushots.com/rest/get_member_challenges', {
            'filter': 'open'
        })
...
#       print "new process ", challenge['title'], ' action ', action, ' value ', value, ' at ', at, ' left ', left
       ...
       try:
           ...
           if 'left' in args and args.left:
               ...
               timeleft = challenge_details["challenge"]["time_left"];
    ...
    def displayChallenge(self, challenge, args):
        ...
        name = self.challenge_details["challenge"]["title"]

        timeleft = self.challenge_details["challenge"]["time_left"];
        timeLeft = "{}D {}H {}M".format(timeleft["days"], timeleft["hours"], timeleft["minutes"])
        timeEnd  = datetime.fromtimestamp(self.challenge_details["challenge"]["close_time"]).strftime(
                    "%d/%m/%Y, %H:%M")

        if not args.all:
            if (timeleft["days"] != 0 or timeleft["hours"] != 0 and  timeleft["minutes"] != 0 or timeleft["seconds"] != 0) and (self.challenge_details["challenge"]["member"]['is_joined'] == True):
                try:
                    vote_data = self.get_votes_panel(self.challenge_details["challenge"]["url"])
                    jauge = ''
                    vote_exposure_factor = ''
                    rank = ''
                    vote = ''
                    total_votes =  self.challenge_details["challenge"]["votes"]
                    total_players = self.challenge_details["challenge"]["players"]
                    timeleft = self.challenge_details["challenge"]["time_left"];
                    timeLeftString = str("{}D:{}H:{}M".format(timeleft["days"], timeleft["hours"], timeleft["minutes"]))
                    timeEnd = datetime.fromtimestamp(self.challenge_details["challenge"]["close_time"]).strftime("%d/%m/%Y, %H:%M")
                    ...
                    print ('challenge : ', name, '(', challenge, ')', 'jauge', jauge, 'end at', timeEnd)
    ...
    def vote_challenge(self, challenge, votes):
        challenge_details = self.get_challenge(challenge)
        if challenge_details["challenge"]["close_time"] != 0 and challenge_details["member"]["is_joined"] == True:
            print(challenge,  "voting", str(votes))
    ...
    def fill_challenge(self, challenge, fill, args):
        #challenge_details = self.get_challenge(challenge)
        #if challenge_details["challenge"]["close_time"] != 0:
        vote_data = self.get_votes_panel(challenge)
        #    print('Filling ' + challenge_details["challenge"]["title"] + ' exposure : ' + str(vote_data["member"]["voting_power"]))

        #    if vote_data["voting"]["exposure"]["exposure_factor"] < args.below:
...
#            for challenge in challenges_open["items"]:
        ...
        if args.end:
            ...
            for section in challenges:
                if args.challenge in '*' or args.challenge in section:
                    # add end of challenge
                    for step in self.strategies['end4'].keys():
                        cmd = ' --cha ' + str(section) + ' ' + self.strategies['end4'][step]
                        cmd_args = self.parser.parse_args(cmd.split())
                        cmd_args.cmde = cmd
                        cmd_args.func(cmd_args)
    ...
    def audience_thread(self):
        ...
        client = InfluxDBClient(url="http://lo-influx-liveobjects.kermit-noprod-b.itn.intraorange", token=token)
    ...
    def ranking_thread(self, challenge):
        ...
        while stillRankingRunning:
            ...
            challenge_details = self.get_challenge(challenge);
            timeleft = challenge_details["items"]["challenge"]["time_left"]
            print('ranking', challenge, timeleft)
    ...
    def ranking_member_old(self, challenge, following, init: False):
        ...
        db_ranking[0]["tags"]['challenge'] = challenge["items"]["challenge"]["url"]
        ...
        if total_rank < int(
                ranking["followers"][following["member"]["user_name"]]["top-rank"]) or total_rank_level > int(
        ...
        for i in range(len(following["entries"])):
            ...
            if ranking["followers"][following["member"]["user_name"]]["entries"].get(str(i)) == None:
                ...
                ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['id'] = \
                following["entries"][i]['id']
                ...
                if init == True:
                    ...
                    self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                                     total_rank, total_rank_level, following["total"]["votes"], timeAtString,
                                     timeLeftString, 'posted', following["entries"][i]['id'],
                                     following["entries"][i]['votes'])
                    ...
                    self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                                     total_rank, total_rank_level, following["total"]["votes"], timeAtString,
                                     timeLeftString, 'post', following["entries"][i]['id'],
                                     following["entries"][i]['votes'])

            else:
                if ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['id'] != \
                        following["entries"][i]['id']:
                    # swapp existente
                    #                    ranking["followers"][following["member"]["user_name"]]["events"][
                    ...
                    ranking["followers"][following["member"]["user_name"]]["events"][
                        "swapped-rank-" + ranking["followers"][following["member"]["user_name"]]["entries"][str(i)][
                            'id']] = self.get_total_rank(following)

                    ranking["followers"][following["member"]["user_name"]]["events"][
                        "swapped-at-" + ranking["followers"][following["member"]["user_name"]]["entries"][str(i)][
                            'id']] = timeAtString

                    ranking["followers"][following["member"]["user_name"]]["events"][
                        "swapped-left-" + ranking["followers"][following["member"]["user_name"]]["entries"][str(i)][
                            'id']] = timeLeftString

                    ranking["followers"][following["member"]["user_name"]]["events"][
                        "swapped-lu-" + ranking["followers"][following["member"]["user_name"]]["entries"][str(i)][
                            'id']] = self.get_total_lu(following)

                    self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                                     total_rank, total_rank_level, self.get_total_votes(following), timeAtString,
                                     timeLeftString, 'swapped',
                                     ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['id'],
                    ...
                    following["entries"][i]['id']
                    ranking["followers"][following["member"]["user_name"]]["events"][
                        "post-" + following["entries"][i]['id'] + '-at'] = timeAtString
                    ranking["followers"][following["member"]["user_name"]]["events"][
                        "post-" + following["entries"][i]['id'] + '-left'] = timeLeftString
                    db_ranking[0]["tags"]['event'] = 'post'
                    self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                                     total_rank, total_rank_level, self.get_total_votes(following), timeAtString,
                                     timeLeftString, 'post',
            ...
            if ranking["followers"][following["member"]["user_name"]]["entries"][str(i)].get('guru_pick') and \
                    ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['guru_pick'] == False and \
                    following["entries"][i]["guru_pick"] == True:
                ranking["followers"][following["member"]["user_name"]]["events"][
                    'gp-' + following["entries"][i]['id'] + '-at'] = timeAtString
                ranking["followers"][following["member"]["user_name"]]["events"][
                    'gp-' + following["entries"][i]['id'] + '-left'] = timeLeftString
                db_ranking[0]["tags"]['event'] = 'gp'
                self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                                 total_rank, total_rank_level, following["total"]["votes"], timeAtString,
                                 timeLeftString, 'gp', following["entries"][i]['id'], following["entries"][i]['votes'])

            ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['guru_pick'] = \
            following["entries"][i]["guru_pick"]

            if ranking["followers"][following["member"]["user_name"]]["entries"][str(i)].get('boost') and \
                    ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['boost'] == False and \
                    following["entries"][i]["boost"] == True:
                ranking["followers"][following["member"]["user_name"]]["events"][
                    'boost-' + following["entries"][i]['id'] + '-at'] = timeAtString
                ranking["followers"][following["member"]["user_name"]]["events"][
                    'boost-' + following["entries"][i]['id'] + '-left'] = timeLeftString
                db_ranking[0]["tags"]['event'] = 'boost'
                self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                                 total_rank, total_rank_level, following["total"]["votes"], timeAtString,
                                 timeLeftString, 'boost', following["entries"][i]['id'],
                                 following["entries"][i]['votes'])

            ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['boost'] = \
            following["entries"][i]["boost"]
            self.ranking_log(challenge["items"]["challenge"]["url"], following["member"]["name"].encode('utf8'),
                             total_rank, total_rank_level, following["total"]["votes"], timeAtString, timeLeftString,
                             'log',
    ...
    def get_entrie(self, entries, id):
        index =-1
        for i in range(len(entries)):
            if entries[str(i)]['id'] == id:
                return i;
        return index

    def get_new_entrie(self, entries, id):
        index =-1
        for i in range(len(entries)):
            if entries[i]['id'] == id:
                return i;
    ...
    def ranking_member(self, challenge, following, init: False):
        ...
        if ranking['followers'].get(following["member"]["user_name"]) == None:
            ...
            ranking["followers"][following["member"]["user_name"]]["events"] = {}
            ranking["followers"][following["member"]["user_name"]]["photos"] = {}
            ranking["followers"][following["member"]["user_name"]]["swapped"] = {}
            ranking["followers"][following["member"]["user_name"]]["swap"] = {}
            ranking["followers"][following["member"]["user_name"]]["entries"] = {}
            ...
            ranking["followers"][following["member"]["user_name"]]["votes"] = 0
            ranking["followers"][following["member"]["user_name"]]["total-votes"] = 0


        ranking["followers"][following["member"]["user_name"]]["total-votes"] = self.get_total_votes(following);


        db_ranking = json_rank_body
        db_ranking[0]['measurement'] = 'ranking'
        db_ranking[0]["tags"]['challenge'] = challenge["items"]["challenge"]["url"]
        db_ranking[0]["tags"]['event'] = 'voting';
        db_ranking[0]["tags"]['player'] = following["member"]["name"].encode('utf8');
        db_ranking[0]["time"] = timeAtString
        db_ranking[0]["fields"]['at'] = timeAtString
        db_ranking[0]["fields"]['left'] = timeLeftString
        db_ranking[0]["fields"]['total-votes'] = self.get_total_votes(following)

        if self.player == 'ranking' and total_votes != int(ranking["followers"][following["member"]["user_name"]]["total-votes"]):
            self.client.write_points(db_ranking);

        # ranking
        # reecherche les swappés
        for i in range(len(ranking["followers"][following["member"]["user_name"]]["entries"])):
            index = self.get_new_entrie( following["entries"],
                ranking["followers"][following["member"]["user_name"]]["entries"][str(i)]['id'])
            if (index == -1):
                # swapped
                ranking["followers"][following["member"]["user_name"]]["events"][timeLeftString + '-swapped-' +
                                                                                 ranking["followers"][
                                                                                     following["member"]["user_name"]][
                                                                                     "entries"][str(i)][
                                                                                     'id'] + '-at'] = timeAtString

        # rechecherche identiques
        for i in range(len(following["entries"])):
            index = self.get_entrie(ranking["followers"][following["member"]["user_name"]]["entries"], following["entries"][i]['id'])
            if index == -1:
                # post
                ranking["followers"][following["member"]["user_name"]]["events"][
                    timeLeftString + '-post-' + following["entries"][i]['id'] + '-at'] = timeAtString
            else:
                # boost
                if ranking["followers"][following["member"]["user_name"]]["entries"][str(index)][
                    'boost'] != 1616217023 and \
                        following["entries"][i]["boost"] == 1616217023:
                    ranking["followers"][following["member"]["user_name"]]["events"][
                        timeLeftString + '-boost-' + following["entries"][i]['id'] + '-at'] = timeAtString

        # recherche des boostés
        for i in range(len(following["entries"])):
            index = self.get_entrie(ranking["followers"][following["member"]["user_name"]]["entries"], following["entries"][i]['id'])
            if index == -1:
                # boost
                if following["entries"][i]["boost"] != -1:
                    ranking["followers"][following["member"]["user_name"]]["events"][
                        timeLeftString + '-boost-' + following["entries"][i]['id'] + '-at'] = timeAtString
            else:
                # boost
                if ranking["followers"][following["member"]["user_name"]]["entries"][str(index)][
                    'boost'] == '-1' and \
                        following["entries"][i]["boost"] != -1:
                    ranking["followers"][following["member"]["user_name"]]["events"][timeLeftString + '-boost-' + following["entries"][i]['id'] + '-at'] = timeAtString

        # mise à jour des entries
        for i in range(len(following["entries"])):
            if ranking["followers"][following["member"]["user_name"]]["entries"].get(str(i)) == None:
                # sir entrie(i) null -> post
                ranking["followers"][following["member"]["user_name"]]["entries"][str(i)] = {}
    ...
    #https: // api.gurushots.com / rest / get_my_active_challenges
    ...
    def get_member(self, challenge):
        return self.session.post('https://api.gurushots.com/rest/get_page_data', data={
    ...
    def submit_to_challenge(self, challenge_id, photo_id):
        ...
        return self.session.post('https://api.gurushots.com/rest/submit_to_challenge', data=payload)
    ...
    def boost_photo(self, challenge_id, photo_id):
        ...
        return self.session.post('https://api.gurushots.com/rest/boost_photo', data=payload)
    ...
    def swap_photo(self, challenge_id, photo_id, new_photo_id):
                ...
                response = self.session.post('https://api.gurushots.com/rest/swap', data=payload)
    ...
    def unlock_key(self, challenge_id, boost):
            ...
            return response_panel('https://api.gurushots.com/rest/key_unlock', data=payload)
            #content = response.content
            #return json.loads(response)
    ...
    def get_challenge_followings(self, c_id):
        return  self.aio_post('https://api.gurushots.com/rest/get_top_photographer', data={
            'c_id': c_id,
            'filter': 'following',
            'init': 'true',
            'limit': 200,
    ...
    def get_followings(self, id, args):
        return self.aio_post('https://api.gurushots.com/rest/get_following', data={
            'id': id,
            'limit': args.limit,
            'start': args.start
        })
    ...
    def get_member_id(self):
        response_panel = self.aio_post('https://api.gurushots.com/rest/get_page_data', data={
    ...
    def get_following_photos(self, id, args):
        ...
        try:
            response_panel = self.aio_post('https://api.gurushots.com/rest/get_top_photos', {
                'id': id,
                'filter': 'following',
                'limit': 200,
                'start': 0
    ...
    def post_votes( self, challenge_details, votes):
        ...
        payload['c_id'] = challenge_details["challenge"]["id"]
