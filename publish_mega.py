...
if __name__ == "__main__":
    home = path
    config = ConfigObj(home+'/gsgui.ini')
    mega = Mega()
    m = mega.login('gyp6band@gmail.com', 'Bruno9798$')
    filename = home + '/dist/' + 'GS.dmg'
    toFilname = home + '/dist/' + 'GS' + '-' + config.get('version') + '-' + datetime.now().strftime('%Y%m%d%H%M')  + '.dmg'

    #os.rename( filename, toFilname)

    print('Uploading ', toFilname)
    #link = m.upload(toFilname)
    # Use it in get_link function
    fileid =  m.find('Gyp6Band' + '-' + config.get('version')+'.dmg')
    #print(link)
    ...
    subject = 'Gyp6Band' + '-' + config.get('version')
    content = link
