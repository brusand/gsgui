...
def ensemble_vote(first_id, first_data, second_id, second_data, algorithms=['bruno_custom', 'hybrid', 'votes_ratio']):
    ...
    for algo in algorithms:
        try:
            ...
            elif algo == 'position_aware':
                choice, ratio1, ratio2, votes, reason = position_aware_algorithm(first_id, first_data, second_id, second_data)
                individual_choices[algo] = choice
                vote_details[algo] = reason
                
            elif algo == 'adaptive_time':
                # Pour adaptive_time, on utilise un temps par défaut car l'ensemble n'a pas accès au time_left
                choice, ratio1, ratio2, votes, reason = adaptive_time_algorithm(first_id, first_data, second_id, second_data, time_left="0D 12H 0M 0S")
                individual_choices[algo] = choice
                vote_details[algo] = reason
